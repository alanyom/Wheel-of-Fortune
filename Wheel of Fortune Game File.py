WHEEL OF FORTUNE PYTHON GAME FULL
...
class WOFHumanPlayer(WOFPlayer):                         
    def getMove(self, catagory, obscuredPhrase, guessed):
        playerMoves = input('''{} has ${}
                    Category: {}
                    Phrase:  {}
                    Guessed: {}

                    Guess a letter, phrase, or type 'exit' or 'pass':'''.format(self.name, self.prizeMoney, self.catagory, self.obscuredPhrase, self.guesses))
        return playerMoves
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if self.smartCoinFlip() is True:
            for x in self.SORTED_FREQUENCIES[:-1]:
                if x in possible:
                    return x
        else:
